...
m1 <- ulam(
  alist(
    ...
    log(lambda) <- a + gr[origin,1]  + gr[destination,2] + 
      ...
      b_pri_o*pri_o + b_pri_d*pri_d +
      b_d*log_distance,
    c(a, b_pop_o, b_pop_d, b_hom_o, b_hom_d, b_soc_o, b_soc_d, b_pri_o, b_pri_d) ~ normal(0,1),
    b_d ~ dnorm(-1,1),
...
precis( m1 )
precis( m1 , depth=3 , pars=c("Rho_gr","sigma_gr") )
...
plot( NULL , xlim=c(0,30) , ylim=c(0,30) , xlab="generalized origin" ,
