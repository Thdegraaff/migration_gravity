...
\begin{frame}{This paper}
	\begin{description}
		...
		\newline
		\item[Aim] To \alert{predict} all changes in incoming and outcoming migration flows of, e.g., Amsterdam, when housing market structure changes \alert{whilst} accounting for origin and destination specific effects \citep{ranjan2007bayesian}
	\end{description}
\end{frame}
...
\begin{itemize}
	\item Frequently used---increases model \alert{performance} and \alert{flexibility} \pause
    \item \alert{Simultenous} modeling at various levels (e.g., cities, regions, flows, individuals) 
    \begin{itemize}
    	\item No two-stage models anymore \pause
    \end{itemize}
    \item Many definitions (in a \alert{Bayesian} context):
	    \begin{itemize}
	    \item mixed effects, varying intercept/parameter, shrinkage, partial pooling models\pause
	    \end{itemize}
	\item \alert{Estimates} the level effects from a distribution, usually ($o_i, d_j) \sim \text{ Normal}(\alpha, \sigma)$
	\begin{itemize}
